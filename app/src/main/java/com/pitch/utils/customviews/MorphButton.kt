...
class MorphButton @JvmOverloads constructor(
    ...
    var fromBgColor: Int = getColorX(R.color.start_orange_color)
    var toBgColor: Int = getColorX(R.color.end_orange_color)
    var fromTextColor: Int = Color.BLACK
    var toTextColor: Int = getColorX(R.color.white)
    var btnRadius = 24 * dp()

    private fun dp(): Float {
        return 24f
    }

    private fun sp(): Float {

        return 24f
    }

    private var iconDrawable = getDrawableX(R.drawable.ic_home)
        set(value) {
            field = value
            setSizeIcon()
        }

    private fun getDrawableX(icSync: Int): Drawable? {
        return ContextCompat.getDrawable(this.context, icSync)
    }
    ...
    private fun setSizeIcon() {
        sizeIcon = iconDrawable?.let {
            it.intrinsicWidth + iconPadding
        }!!
    }


    private fun getColorX(color: Int): Int {
